        ...
        <div className="grid items-start gap-[clamp(36px,5vw,72px)] lg:grid-cols-[minmax(0,1.55fr)_minmax(0,1fr)]">
          <div className="flex flex-col gap-4.5">
            {profile.bio.map((paragraph, i) => (
              <p key={i} className="m-0 [text-wrap:pretty] text-justify">
                {paragraph[locale]}
              </p>
            ))}
            <a
              ...
              className="inline-flex self-start w-fit items-center gap-2.5 rounded-full bg-accent-strong px-6 py-3 text-[15px] font-semibold text-on-accent no-underline transition-colors hover:bg-green hover:text-on-green"
            >
          ...
          <aside className="rounded-[18px] border border-line bg-paper p-[clamp(24px,3vw,32px)] shadow-soft">
            <h3 className="mb-6 mt-0 font-mono text-[12.5px] font-medium uppercase tracking-[0.16em] text-sage">
              {ui.about.factsTitle[locale]}
            </h3>
            <div className="mb-6">
              <div className="mb-4 font-mono text-[11px] uppercase tracking-[0.12em] text-muted">
                {profile.degree[0]?.label[locale]}
              </div>
              <ul className="m-0 flex list-none flex-col gap-4.5 p-0">
                {profile.degree.map((degree, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-[15.5px] font-medium leading-[1.45]">
                    <span className="mt-[0.35rem] h-2 w-2 shrink-0 rounded-full bg-sage" aria-hidden="true" />
                    <p className="m-0">{degree.value[locale]}</p>
                  </li>
                ))}
              </ul>
            </div>
