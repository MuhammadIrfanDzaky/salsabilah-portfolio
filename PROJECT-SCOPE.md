...
1. **Isi `OPENROUTER_API_KEY` dan `OPENROUTER_MODEL`** di Vercel → Settings → Environment
   Variables (keduanya wajib — tanpa `OPENROUTER_MODEL` fiturnya tetap tidak aktif, karena
   sengaja tidak ada model bawaan di kode). Ini satu-satunya gate blocking yang masih terbuka
   (#29). Sampai dilakukan, tombol terjemahan menjawab "belum dikonfigurasi" dan tidak ada
   biaya yang bisa keluar.
2. **Isi `NEXT_PUBLIC_FORMSPREE_ID`** di Vercel bila formulir kontak memang diinginkan —
   ambil ID-nya dari https://formspree.io (New Form → bagian akhir endpoint `/f/<id>`).
   Dikosongkan **bukan** keadaan rusak: bagian kontak menampilkan tombol kirim email langsung,
   dan itu sudah jalan hari ini. Sebelum mengisinya, lihat catatan privasi di #6 — sejak form
   aktif, nama/email/pesan pengunjung mengalir ke pihak ketiga.
