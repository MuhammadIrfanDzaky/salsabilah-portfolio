...
- **Gates open:** 0 dari 21 blocking — kelima gate langkah 4 (#5, #18, #29, #34, #46) ditutup 2026-07-28
- **Riwayat:** 14 terbuka sebelum langkah 3 · 7 sesudahnya · 5 setelah Fase 2 · 0 setelah langkah 4
  (audit 2026-07-27 sempat menulis 5 padahal 7; angka itu dihitung dari ingatan, bukan dari tabel)
- **Nol gate terbuka bukan berarti siap rilis.** Tiga tindakan manual masih menunggu — lihat
  "Yang belum diverifikasi lewat UI" dan "Tindakan manual" di bawah.
...
Tiga hal yang hanya bisa dikerjakan pemilik, dan tidak satu pun bisa diverifikasi dari kode:

1. **Matikan signup publik** — Supabase Dashboard → Authentication → Sign In / Providers → Email.
   Terbuka sejak dasbor admin bisa dijangkau publik.
2. **Isi `OPENROUTER_API_KEY`** (dan opsional `OPENROUTER_MODEL`) di Vercel → Settings →
   Environment Variables. Sampai itu dilakukan, tombol terjemahan menjawab "belum dikonfigurasi"
   dan tidak ada biaya yang bisa keluar.
3. **Hapus tiga artikel `[DUMMY]`** beserta cover placeholder di `public/blog-covers/` (K8).
